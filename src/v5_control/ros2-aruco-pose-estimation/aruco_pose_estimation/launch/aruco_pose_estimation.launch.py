...
def generate_launch_description():
    ...
    # 新增 camera_name 参数，用于自动生成相机话题
    camera_name_arg = DeclareLaunchArgument(
        name='camera_name',
        default_value='camera',
        description='Camera namespace (e.g., camera_01, camera_02)',
    )
    ...
    # 根据 camera_name 自动生成话题名
    camera_name = LaunchConfiguration('camera_name')
    
    image_topic_arg = DeclareLaunchArgument(
        name='image_topic',
        default_value=PythonExpression(["'/' + '", camera_name, "' + '/color/image_raw'"]),
    ...
    depth_image_topic_arg = DeclareLaunchArgument(
        ...
        default_value=PythonExpression(["'/' + '", camera_name, "' + '/depth/image_raw'"]),
        description='Name of the depth image topic to subscribe to',
    )

    camera_info_topic_arg = DeclareLaunchArgument(
        name='camera_info_topic',
        default_value=PythonExpression(["'/' + '", camera_name, "' + '/color/camera_info'"]),
        description='Name of the camera info topic to subscribe to',
    )

    camera_frame_arg = DeclareLaunchArgument(
        name='camera_frame',
        default_value=PythonExpression(["'", camera_name, "' + '_color_optical_frame'"]),
